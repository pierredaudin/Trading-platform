...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, Trader4)
...
def prix_actuel_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, Trader4)
        agent_object.prix_actuelI = value
        # add code here if needed
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    igs.input_create("prix_actuel", igs.INTEGER_T, None)
        ...
        while (not is_interrupted) and igs.is_started():
            while (not is_interrupted) and igs.is_started():
                try:
                    ...
                    type_ordre, quantite = agent.prendre_decision()
                    
                    # Utiliser le service pour envoyer l'ordre à la banque
                    print(f"[DEBUG] Envoi d'un ordre à la banque : {type_ordre} {quantite} jetons.")
