...
from Trader_Manuel import *

port = 5670
agent_name = "Trader_Manuel"
device = None
verbose = False
...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, TraderManuel)
...
# inputs
def prix_actuel_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, TraderManuel)
        agent_object.prix_actuel = value
        agent_object.update_price_label()  # Mettre à jour l'affichage du prix dans l'interface graphique
    except:
        print(traceback.format_exc())

# services
def reponse_info_compte_callback(sender_agent_name, sender_agent_uuid, service_name, tuple_args, token, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, TraderManuel)
        solde = tuple_args[0]
        jetons = tuple_args[1]
        agent_object.reponse_info_compte(sender_agent_name, sender_agent_uuid, solde, jetons)
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    igs.input_create("prix_actuel", igs.DOUBLE_T, None)

    igs.observe_input("prix_actuel", prix_actuel_input_callback, agent)

    igs.service_init("reponse_info_compte", reponse_info_compte_callback, agent)
    igs.service_arg_add("reponse_info_compte", "solde", igs.DOUBLE_T)
    igs.service_arg_add("reponse_info_compte", "jetons", igs.INTEGER_T)
